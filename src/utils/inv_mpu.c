...
/**
 *  @brief      Parse one continuous block of data.
 *  @param[in] bfr        Data in, 22 bytes
 *  @param[out] acc   accelerometer reading, length 3
 *  @param[out] gyro  gyroscope reading, length 3
 *  @param[out] mag   magnetometer reading, length 3
 *  @param[out] temp  temperature reading, length 1
 *  @return     0 if successful.
 */
int mpu_parse_all_sensors(unsigned char *bfr,
					     short *acc, short *gyro, 
                         short *mag, long *temp)
{
	//total size = 22
	...
	// temperature
    short raw;
    raw = (bfr[0+offset] << 8) | bfr[1+offset];
    temp[0] = (long)((21 + ((raw - (float)st.hw->temp_offset) / st.hw->temp_sens)) * 65536L);
	offset += 2;
	
	// gyro
    gyro[0] = (bfr[0+offset] << 8) | bfr[1+offset];
    gyro[1] = (bfr[2+offset] << 8) | bfr[3+offset];
    gyro[2] = (bfr[4+offset] << 8) | bfr[5+offset];
	offset += 6;
	
	// meganeto 	
#if defined AK8975_SECONDARY
    /* AK8975 doesn't have the overrun error bit. */
    if (!(bfr[0+offset] & AKM_DATA_READY))
        return -2;
    if ((bfr[7+offset] & AKM_OVERFLOW) || (bfr[7+offset] & AKM_DATA_ERROR))
        return -3;
#elif defined AK8963_SECONDARY
    /* AK8963 doesn't have the data read error bit. */
    if (!(bfr[0+offset] & AKM_DATA_READY) || (bfr[0+offset] & AKM_DATA_OVERRUN))
        return -2;
    if (bfr[7+offset] & AKM_OVERFLOW)
        return -3;
#endif
    mag[0] = (bfr[2+offset] << 8) | bfr[1+offset];
    mag[1] = (bfr[4+offset] << 8) | bfr[3+offset];
    mag[2] = (bfr[6+offset] << 8) | bfr[5+offset];
 ...
 *  @return     0 if successful.
 */
int mpu_get_all_sensors(short *acc, short *gyro, 
                       short *mag, long *temp, 
					   unsigned long *timestamp)
{
	...
	// Read all data, see parse function for more info
    ...
    if (timestamp)
        get_ms(timestamp);
	return mpu_parse_all_sensors(tmp, acc, gyro, mag, temp);
}
 ...
 *  @return     0 if successful.
 */
int mpu_ask_all_sensors()
{
	if(arduino_pdci2c_ask(st.hw->addr, st.reg->raw_accel, 22))
		return -1;
	return 0;
}
